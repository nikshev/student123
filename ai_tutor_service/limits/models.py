# impl: FR-002-14
"""
DailyCounter, IdempotencyRecord, and ReservationRecord models for AI Tutor Service.
...
ReservationRecord: request_id PK, user_id, date_utc, daily_remaining, created_at.
Tracks quota reservations for idempotency and atomicity.
"""
...
class ReservationRecord(models.Model):
    """
    Reservation record for daily quota idempotency.
    request_id is the primary key.
    Stores the quota reservation outcome for durable idempotency.
    """

    request_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user_id = models.CharField(max_length=255, blank=False, null=False)
    date_utc = models.DateField(blank=False, null=False)
    daily_remaining = models.PositiveIntegerField(
        default=0,
        validators=[MinValueValidator(0)],
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        indexes = [
            models.Index(fields=["user_id", "date_utc"]),
            models.Index(fields=["date_utc"]),
            models.Index(fields=["user_id", "created_at"]),
        ]

    def __str__(self):
        return f"ReservationRecord({self.request_id} user={self.user_id} date={self.date_utc} remaining={self.daily_remaining})"

    def clean(self):
        super().clean()
        if self.daily_remaining < 0:
            raise ValidationError({"daily_remaining": "daily_remaining cannot be negative."})